<div class="sf_admin_list">
    <table class="datatable table table-bordered table-striped" id="table_vtp_manage_products_category" style="margin-top: 5px !important;">
      <thead>
          <tr>
      ...
      <!--   End thongnq1   -->
        <?php foreach ($results as $i => $vtp_products_category): $odd = fmod(++$i, 2) ? 'odd' : 'even' ?>

          <tr class="sf_admin_row <?php echo $odd ?> {pk: <?php echo $vtp_products_category->getId() ?>}" rel="<?php echo $vtp_products_category->getId() ?>">

                            <?php include_partial('vtpManageProductsCategory/list_td_batch_actions', array('vtp_products_category' => $vtp_products_category, 'helper' => $helper)) ?>
                            <?php $orderNo = ($i) + ($currentPage - 1)*$pager->getMaxPerPage(); ?>
                <?php include_partial('vtpManageProductsCategory/list_td_tabular', array('vtp_products_category' => $vtp_products_category, 'orderNo' => $orderNo)) ?>
                            <?php include_partial('vtpManageProductsCategory/list_td_actions', array('vtp_products_category' => $vtp_products_category, 'helper' => $helper)) ?>
                      </tr>
        <?php endforeach; ?>
      </tbody>
      <tfoot>
        <tr>
          <th colspan="5">
            ...
            <div style="position: relative; width: auto; float:right"><?php include_partial('vtpManageProductsCategory/pagination', array('pager' => $pager)) ?></div>
