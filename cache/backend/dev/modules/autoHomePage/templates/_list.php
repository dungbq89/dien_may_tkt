<div class="sf_admin_list">
    <table class="datatable table table-bordered table-striped" id="table_home_page" style="margin-top: 5px !important;">
      <thead>
          <tr>
          ...
          <?php include_partial('homePage/list_th_tabular', array('sort' => $sort)) ?>
      ...
      <!--   End thongnq1   -->
        <?php foreach ($results as $i => $sf_guard_user): $odd = fmod(++$i, 2) ? 'odd' : 'even' ?>
          ...
          <tr class="sf_admin_row <?php echo $odd ?> {pk: <?php echo $sf_guard_user->getId() ?>}" rel="<?php echo $sf_guard_user->getId() ?>">

                            <?php include_partial('homePage/list_td_batch_actions', array('sf_guard_user' => $sf_guard_user, 'helper' => $helper)) ?>
                            <?php $orderNo = ($i) + ($currentPage - 1)*$pager->getMaxPerPage(); ?>
                <?php include_partial('homePage/list_td_tabular', array('sf_guard_user' => $sf_guard_user, 'orderNo' => $orderNo)) ?>
                            <?php include_partial('homePage/list_td_actions', array('sf_guard_user' => $sf_guard_user, 'helper' => $helper)) ?>
                      </tr>
        <?php endforeach; ?>
      </tbody>
      <tfoot>
        <tr>
          <th colspan="19">
            ...
            <div style="position: relative; width: auto; float:right"><?php include_partial('homePage/pagination', array('pager' => $pager)) ?></div>
