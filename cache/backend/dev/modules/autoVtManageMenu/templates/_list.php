<div class="sf_admin_list">
    <table class="datatable table table-bordered table-striped" id="table_vt_manage_menu" style="margin-top: 5px !important;">
      <thead>
          <tr>
          ...
          <?php include_partial('vtManageMenu/list_th_tabular', array('sort' => $sort)) ?>
      ...
      <!--   End thongnq1   -->
        <?php foreach ($results as $i => $vtp_menu): $odd = fmod(++$i, 2) ? 'odd' : 'even' ?>

          <tr class="sf_admin_row <?php echo $odd ?> {pk: <?php echo $vtp_menu->getId() ?>}" rel="<?php echo $vtp_menu->getId() ?>">

                            <?php include_partial('vtManageMenu/list_td_batch_actions', array('vtp_menu' => $vtp_menu, 'helper' => $helper)) ?>
                            <?php $orderNo = ($i) + ($currentPage - 1)*$pager->getMaxPerPage(); ?>
                <?php include_partial('vtManageMenu/list_td_tabular', array('vtp_menu' => $vtp_menu, 'orderNo' => $orderNo)) ?>
                            <?php include_partial('vtManageMenu/list_td_actions', array('vtp_menu' => $vtp_menu, 'helper' => $helper)) ?>
                      </tr>
        <?php endforeach; ?>
      </tbody>
      <tfoot>
        <tr>
          <th colspan="7">
            ...
            <div style="position: relative; width: auto; float:right"><?php include_partial('vtManageMenu/pagination', array('pager' => $pager)) ?></div>
