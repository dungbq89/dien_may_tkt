...
<div class="shop-page-title category-page-title page-title ">
    ...
    <div class="page-title-inner flex-row  medium-flex-wrap container">
        <div class="flex-col flex-grow medium-text-center">
            <div class="is-small">
                <nav class="woocommerce-breadcrumb breadcrumbs"><a href="<?php echo url_for1('homepage') ?>">Trang chủ</a> <span
                            class="divider">&#47;</span> <?php echo $category->name ?>
                </nav>
            </div>
                            ...
                            <ul>
                                <?php if ($minPrice): ?>
                                    <li class="chosen"><a aria-label="Xóa bộ lọc"
                                                          href="<?php echo url_for1(sprintf('@cat_product?slug=%s&%s', $slug, VtHelper::getUrlCat($page, false, $maxPrice, $orderby))) ?>"
                                ...
                                <?php if ($maxPrice): ?>
                                    <li class="chosen"><a aria-label="Xóa bộ lọc"
                                                          href="<?php echo url_for1(sprintf('@cat_product?slug=%s&%s', $slug, VtHelper::getUrlCat($page, $minPrice, false, $orderby))) ?>"
            ...
            <form class="woocommerce-ordering" action="" method="get">
                ...
                <?php if ($minPrice): ?>
                    <input type="hidden" name="min_price" value=""
                           data-min="<?php echo $minPrice ? $minPrice : 0 ?>"
                ...
                <div class="products row row-small large-columns-4 medium-columns-3 small-columns-2 has-shadow row-box-shadow-1 row-box-shadow-2-hover">
                    ...
                    <?php if (count($products)): ?>
                        <?php foreach ($products as $pro): ?>
                            ...
                            $link = url_for1(sprintf('@productDetail?cat_slug=%s&slug=%s', $slug, $pro->slug));
                        ...
                        <nav class="woocommerce-pagination">
                            <ul class="page-numbers nav-pagination links text-center">
                                <?php if ($pager->getPage() >= 2): ?>
                                    <li><a class="prev page-number" href="<?php echo url_for1(sprintf('@cat_product?slug=%s&%s', $slug, VtHelper::getUrlCat('', false, $maxPrice, $orderby))) ?>"><i
                                                    class="icon-angle-left"></i></a></li>
                                <?php endif; ?>
                                <?php foreach ($pager->getLinks() as $pg): ?>
                                    <?php if ($pg == $pager->getPage()): ?>
                                        <li><span aria-current="page" class="page-number current"><?php echo $pager->getPage() ?></span></li>
                                    <?php else: ?>
                                        <li><a class="page-number" href="<?php echo url_for1(sprintf('@cat_product?slug=%s&%s', $slug, VtHelper::getUrlCat($pg, false, $maxPrice, $orderby))) ?>"><?php echo $pg ?></a>
                                        </li>
                                    <?php endif; ?>
                                <?php endforeach; ?>

                                <?php if ($pager->getPage() < $pager->getLastPage()): ?>
                                    <li><a class="next page-number" href="<?php echo url_for1(sprintf('@cat_product?slug=%s&%s', $slug, VtHelper::getUrlCat($pager->getLastPage(), false, $maxPrice, $orderby))) ?>"><i
        ...
        <div id="shop-sidebar" class="mfp-hide">
            <div class="sidebar-inner">
                ...
                <aside id="woocommerce_price_filter-2" class="widget woocommerce widget_price_filter"><span
                    ...
                    <form method="get"
                          action="<?php echo url_for1(sprintf('@cat_product?slug=%s&page=%s', $slug, $page)) ?>">
